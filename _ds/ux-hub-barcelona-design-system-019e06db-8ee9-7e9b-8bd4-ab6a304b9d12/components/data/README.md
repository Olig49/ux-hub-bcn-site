...
- **`dt`:** `font: 500 13px/1.2`, `letter-spacing: 0.04em`, uppercase, `color: #6B6B6B`.
...
- **`.sp-stat-sub`:** the smaller trailing label inside a `dd` ("from 310 ratings", "every
  meetup, always") — `font: 400 14px/1.3`, `color: #6B6B6B`.
...
**Found in:** sponsors.html:161-165 — "Members on Meetup" (2,816), "Group rating" (4.7, from 310
ratings), "Cost to attend" (Free, every meetup, always).
...
the accessibility question. The `#6B6B6B` used for `dt` and `.sp-stat-sub` is the same
**uncorrected** literal noted in `cards/README.md` for `.sponsor-card p` — not the `--uxh-fg-2`
token (`#6a6a6a`) the spec's §2 accessibility correction names; both should converge on the token.
...
(sponsors.html:363-377) — the same daily-refreshed data source the homepage's EventFeature and
...
the outer border. `var(--uxh-fg-2)` should replace the hardcoded `#6B6B6B` on `dt` and
`.sp-stat-sub` once that reconciliation happens (see Accessibility above). No radius token exists
for `18px` — it sits between `var(--uxh-radius-lg)` (20px) and `var(--uxh-radius-md)` (12px) as
its own one-off value; not worth adding a new token for a single 18px instance.
